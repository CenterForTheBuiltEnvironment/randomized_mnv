...
conv_saving <- baseline_results %>% 
  left_join(df_interv %>% filter(datetime >= as.Date("2022-01-01")), by = "datetime") %>% 
  mutate(saving = power - towt) %>% 
  mutate(n_weeks = interval(min(datetime), datetime) %>% as.numeric('weeks') %>% floor()) %>%
  filter(n_weeks <= 48)
```

```{r sprtresults, fig.height=10, fig.width=7.5, fig.cap="Comprehensive results summary of the proposed M&V method applied to the case study throughout a total of 60 weeks"}
p2 <- ggplot(sprt_res, aes(x = n_weeks, y = ns_stat)) +
  ...
  geom_hline(yintercept = mean(conv_saving %>% .$saving, na.rm = T), 
             linetype = "dashed", 
             color = "black") +
  annotate(geom = "text",
           x = 5,
...
p3 <- ggplot(annual_saving) +
  ...
  geom_text(data = sprt_overlap_base %>% filter(flag == 1), 
            aes(x = ifelse(is.null(first(flag)), max(sprt_overlap_base$n_weeks), n_weeks), 
                y = ifelse(is.null(first(flag)), max(sprt_overlap_base$annual_saving), annual_saving) + 4, 
  ...
  geom_text(data = sprt_overlap_s2 %>% filter(flag == 1), 
            aes(x = ifelse(is.null(first(flag)), max(sprt_overlap_s2$n_weeks), n_weeks), 
                y = ifelse(is.null(first(flag)), max(sprt_overlap_s2$annual_saving), annual_saving) - 2, 
                label = paste0("Temperature check\n(Intervention)\nweek ", n_weeks)), 
            colour = "grey20", size = 2.5, fontface = "italic") +
  ...
  geom_text(data = annual_saving %>% filter(n_weeks == 12),
            aes(x = n_weeks, y = annual_saving + 3,label = paste0("Minimum\ntest length check\nweek ", n_weeks)),
            colour = "grey20", 
            size = 2.5, 
            fontface = "italic")+
  ...
  geom_text(data = sprt_res %>% filter(n_weeks%%12 == 0 & flag == 1) %>% slice(1),
            aes(x = n_weeks, y = annual_saving - 2, label = paste0("SPRT check\nweek ", n_weeks)), 
            colour = "grey20", 
            size = 2.5, 
            fontface = "italic") +
  geom_vline(xintercept = seq(0, 50, by = 12), 
...
b). The difference between baseline measurements and intervention
measurements was detected early in week 12. However, subplot c)
indicates that outdoor weather has not reached the 80% threshold range
calculated with reference to the typical meteorological year (TMY)
weather file, we need to continue the M&V. At week 16, the accumulated
...
we report a 6.6 kW savings at the end of this blocking period (i.e. week
...
savings stabilize at 12%. If this is a real project, and the analyst
follows the steps outlined in the standards, week 24 is merely the
mid-point of baseline collection assuming no interruptions or delays and
he/she has to wait for another one year and half to estimate savings.
Subplot b) also shows the saving estimated from a conventional 2-year
pre-/post- type M&V, which is similar to the results obtained at week 24
and onward if the analyst applies the novel randomized M&V.
...
after the retrofit implementation, the energy consumption decreased by
10 kW for each metered interval from April to September because of
decreased occupancy (i.e. tenants moving out leaving one floor vacant)
and that the analyst was not aware of this change in occupancy.
...
an outdoor weather and time-dependent regression model using collected
metered data as the baseline. In other words, the model is only valid
for stable occupancy. Then the building manager enables the intervention
control in the Building Automation System (BAS) and configures the
existing baseline control as a backup when the intervention fails for
safekeeping. Subplot b) in the figure \@ref(fig:SAplot) shows what
happens after implementing the intervention, there is a 10 kW decrease
at every metered interval due to a lower occupancy in the case study
building. In the post-retrofit period, only intervention can be
measured. The projected baseline is the baseline model prediction
results using date/time features and outdoor weather conditions
representing the analyst's estimation in the post-retrofit period. The
correctly adjusted baseline represents the underlying ground truth of
this scenario but unobervable to the analyst, and the figure shows from
April to September, the modeled/predicted baseline deviates from the
correctly adjusted baseline results.
...
depending on a pre-determined schedule. Figure \@ref(fig:SArand) shows
even if the analyst start the M&V during the occupancy change and has no
prior knowledge about when and how long the occupancy has changed, there
is a decent chance that each control strategy sampled 3 months with a
balanced day-of-week category. In this way, at least the non-routine
event has been measured.
...
The results shown in figure \@ref(fig:SAresults) demonstrate that the
conventional pre-/post- method is likely to provide a higher saving
estimation for this scenario if the analyst can not adjust for the
proposed change, but the novel randomized method is robust and requires
no additional assumptions or modeling effort. This is because sampling
random at a 50%/50% ratio, the unexpected disturbance can mostly be
balanced when using difference-in-mean as a saving-effec estimator.
...
# saving calculation
FS_true <- (mean(df_base_alt %>% .$power) - 
            mean(df_interv_alt %>% .$power)) / 
           (mean(df_base_alt %>% .$power)) * 100

FS_new <- (mean(df_hourly_sch %>% filter(strategy == 1) %>% .$power) - 
           mean(df_hourly_sch %>% filter(strategy == 2) %>% .$power)) / 
          (mean(df_hourly_sch %>% filter(strategy == 1) %>% .$power)) * 100

FS_conv <- (mean(baseline_results %>% .$towt) - 
            mean(df_interv_alt %>% .$power)) / 
           (mean(baseline_results %>% .$towt)) * 100
...
clogging, static pressure increases gradually by 50% over the course of
...
power would also increase from 100% to 150%. We demonstrate such
...
Similarly, the above figure \@ref(fig:SBdata) shows both metered energy
consumption and modeled baseline for a 2-year M&V project in this
scenario. The figure shows if using the conventional method, the analyst
would start a 12-month baseline and fit a regression model using the
collected measurements. If the static pressure increase covers the
course of the study period, there is a chance that the fitted model may
indirectly account for the change in baseline in this case. However,
subplot b) shows that the projected modeling results still
under-estimate the fan energy increase compared to correctly adjusted
baseline in the post-retrofit period. Normally the fan energy increase
can be detected using sensors connected to the AHU, but it is outside
the scope for a whole building metered energy consumption M&V. However,
if the analyst choose to apply the random switching method shown in
figure \@ref(fig:SBrand), the energy increase can be measured any time
...
As a result, figure \@ref(fig:SBresults) implies that the conventional
M&V significantly underestimates the intervention energy-saving effect
due to a lack of baseline measurements in the post-retrofit duration.
Additionally as expected, when using the randomized M&V, savings are
...
FS_new <- (mean(df_hourly_alt %>% filter(strategy == 1) %>% .$power) - 
           mean(df_hourly_alt %>% filter(strategy == 2) %>% .$power)) / 
          (mean(df_hourly_alt %>% filter(strategy == 1) %>% .$power)) * 100
